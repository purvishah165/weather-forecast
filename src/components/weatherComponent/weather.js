...
class Weather extends React.Component {
    ...
    return (
      <div className="wrapper">
        <form className="location-form" onSubmit={this.getWeather}>
          ...
          {this.state.error && (
            <span className="weather-error">{this.state.error}</span>
          )}
        </form>
        {!this.state.loading && (
          <div className="main">
            ...
            <div className="days">
              {this.state.days.map(day => (
                <button
                  ...
                  key={day.label}
                  disabled={
                    day.date.toDateString() === this.state.date.toDateString()
                  }
                >
                  <div>
                    {
                      <img
                        ...
                        src={
                          ...
                          day.weatherData.weather[0].icon +
                          ".png"
                        }
                      />
                    }
        ...
        {this.state.loading && <div className="loader cover-spin" />}
      </div>
    );
  }
  componentDidMount() {
  ...
  getWeather = e => {
    ...
    this.props.history.push(
      `/weather/${this.state.city_name ? this.state.city_name : ""}`
    );
  ...
  selectDate = selectedDate => {
    const dayLabel =
      new Date().toDateString() === selectedDate.toDateString()
        ? "Today"
        : Weather.dayLabel(selectedDate.getDay());
